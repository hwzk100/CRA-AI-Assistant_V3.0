...
// ============================================================================
// Worksheet 1: Inclusion/Exclusion Criteria
// ============================================================================
 ...
 */
export const createEditState = <T>(data: T): EditState<T> => ({
  original: data,
  edited: { ...data },
 ...
 */
export const updateEditState = <T>(
  state: EditState<T>,
  updates: Partial<T>
): EditState<T> => {
  const newEdited = { ...state.edited, ...updates };
  const modifiedFields: (keyof T)[] = [];

  for (const key of Object.keys(updates) as (keyof T)[]) {
    if (JSON.stringify(state.original[key]) !== JSON.stringify(updates[key])) {
  ...
  return {
    ...
    edited: newEdited,
    hasUnsavedChanges: modifiedFields.length > 0,
    modifiedFields: [...state.modifiedFields, ...modifiedFields],
  };
};
 ...
 */
export const cloneItem = <T extends { id: string }>(item: T): T => {
  return {
    ...item,
    id: generateId(),
  };
