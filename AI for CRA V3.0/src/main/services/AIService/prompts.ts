...
2. 【重要】提取每个访视的时间窗口（windowStart和windowEnd）- 这是必须字段
3. 提取每个访视的编号和名称：
   - 访视编号应反映方案中的原始格式
...
**时间窗提取规则：**
- **只提取±天数**，例如：±2天、±3天、±7天
- 如果方案中写了周期+容差（如"C1D1 ± 2天"），只提取"±2天"
- 如果方案中写了时间窗范围（如"Day -28 ~ Day -1"），计算天数差后显示（如"±27天"）
- 如果方案中只写了访视点（如"C1D1"、"Day 1"）且没有窗口期，windowStart和windowEnd都填"无"或"0天"
- 从表格列标题提取：如"Day 1±2天" → windowStart: "Day 1", windowEnd: "±2天"
- 从描述文字提取：如"周期1第1天，允许±2天窗口" → windowStart: "C1D1", windowEnd: "±2天"
  ...
  "visitSchedule": [
    {
      ...
      "windowEnd": "±27天",
      "procedures": [
        {"name": "知情同意", "category": "screening", "timing": "Day -28", "required": true}
      ],
      "assessments": [
        {"name": "血常规", "type": "lab", "timing": "Day -1", "required": true}
      ]
    },
    {
      "visitNumber": "0",
...
示例2 - 时间窗提取方式说明：
- 方案写"Day -28 ~ Day -1" → windowStart: "Day -28", windowEnd: "±27天"
- 方案写"C1D1 ± 2天" → windowStart: "C1D1", windowEnd: "±2天"
- 方案写"Day 1"（无窗口） → windowStart: "Day 1", windowEnd: "无"
- 方案写"允许±3天窗口期" → windowStart: "原访视点", windowEnd: "±3天"
- 方案写"C1D1"（周期标识，无容差） → windowStart: "C1D1", windowEnd: "无"
...
【重要】windowStart和windowEnd是必须字段，不能为空或null！

格式说明：
- windowStart：使用方案中的访视时间点原文描述（如 "C1D1", "Day 1", "Week 12"）
- windowEnd：**只显示±天数**，如 "±2天"、"±7天"，如果没有窗口期则填"无"
- 周期访视示例： windowStart: "C1D1", windowEnd: "±2天" 或 "无"
- 筛选期示例： windowStart: "Day -28", windowEnd: "±27天"（计算天数差）
- 基线示例： windowStart: "Day 1", windowEnd: "无"
- 随访示例： windowStart: "Week 12", windowEnd: "±7天"

时间窗提取优先级：
1. **直接提取±天数**：方案中明确写了±容差，直接提取
   - 例："C1D1 ± 2天" → windowStart: "C1D1", windowEnd: "±2天"
   - 例："Week 12 ± 7天" → windowStart: "Week 12", windowEnd: "±7天"

2. **计算范围天数差**：方案中写了时间范围，计算差值
   - 例："Day -28 ~ Day -1" → windowStart: "Day -28", windowEnd: "±27天"
   - 例："Day 1±2天" → windowStart: "Day 1", windowEnd: "±2天"

3. **周期标识提取**：从访视名称中提取周期信息
   - 例：访视名称"C1D1" → windowStart: "C1D1", windowEnd: "无"（除非有容差说明）
   - 例：访视名称"Cycle 2 Day 8" → windowStart: "C2D8", windowEnd: "无"

4. **表格标题提取**：从流程表的列标题提取时间窗
   - 例："Day 1±2天" → windowStart: "Day 1", windowEnd: "±2天"
   - 例："C1D1 (±2天)" → windowStart: "C1D1", windowEnd: "±2天"

5. **描述文字提取**：从访视描述中提取窗口信息
   - 例："允许±2天窗口期" → windowEnd: "±2天"
   - 例："必须在3天内完成" → windowEnd: "±3天"

6. **默认值填充**：如果方案中没有明确时间窗，根据访视类型推断
   - 筛选期：windowStart: "Day -28", windowEnd: "±27天" 或按方案原文
   - 基线：windowStart: "Day 1", windowEnd: "无"
   - 治疗周期：windowStart使用周期标识，windowEnd: "无"（除非有容差）
   - 随访：windowStart: "Week 12", windowEnd按方案或"无"
