...
**时间窗提取规则：**
- 如果方案中明确写了时间窗（如"Day -28 ~ Day -1"），直接提取
- 如果方案中写了周期+容差（如"C1D1 ± 2天"），windowStart填"C1D1"，windowEnd填"C1D1 ± 2天"
- 如果方案中只写了访视点（如"C1D1"、"Day 1"），windowStart和windowEnd都填相同的值
- 如果方案中没有明确时间窗，根据访视类型推断：
  * 筛选期：通常是 "Day -28 ~ Day -1" 或类似
  * 基线：通常是 "Day 0" 或 "Day 1"
  * 治疗周期：使用周期标识（如"C1D1"）
- 从表格列标题提取：如"Day 1±2天" → windowStart: "Day -1", windowEnd: "Day 3"
- 从描述文字提取：如"周期1第1天，允许±2天窗口" → windowStart: "C1D1", windowEnd: "C1D1 ± 2天"
...
示例2 - 时间窗提取方式说明：
- 方案写"Day -28 ~ Day -1" → windowStart: "Day -28", windowEnd: "Day -1"
- 方案写"C1D1 ± 2天" → windowStart: "C1D1", windowEnd: "C1D1 ± 2天"
- 方案写"Day 1"（无窗口） → windowStart: "Day 1", windowEnd: "Day 1"
- 方案写"允许±3天窗口期" → windowStart: "原访视点", windowEnd: "原访视点 ± 3天"
- 方案写"C1D1"（周期标识） → windowStart: "C1D1", windowEnd: "C1D1"
...
格式说明：
- 使用方案中的原文描述，保留所有格式细节
- 周期访视示例： "C1D1", "Cycle 1 Day 1", "周期1第1天"
- 带容差的周期： "C1D1", "C1D1 ± 2天"（windowEnd包含容差）
- 筛选期示例： "Day -28 ~ Day -1", "筛选期 -28天至-1天"
- 基线示例： "Day 0", "Day 1", "Day 1 (首次给药)"
- 随访示例： "Week 12", "Week 12 ± 7天", "治疗结束后30天"

时间窗提取优先级：
1. **直接提取**：方案中明确写了时间窗范围，直接使用原文
   - 例："Day -28 ~ Day -1" → windowStart: "Day -28", windowEnd: "Day -1"
   - 例："Week 12 ± 7天" → windowStart: "Week 12", windowEnd: "Week 12 ± 7天"

2. **周期标识提取**：从访视名称中提取周期信息
   - 例：访视名称"C1D1" → windowStart: "C1D1", windowEnd: "C1D1"
   - 例：访视名称"Cycle 2 Day 8" → windowStart: "C2D8", windowEnd: "C2D8"

3. **表格标题提取**：从流程表的列标题提取时间窗
   - 例："Day 1±2天" → windowStart: "Day -1", windowEnd: "Day 3"
   - 例："C1D1 (±2天)" → windowStart: "C1D1", windowEnd: "C1D1 ± 2天"

4. **描述文字提取**：从访视描述中提取窗口信息
   - 例："允许±2天窗口期" → 在windowEnd中添加容差说明
   - 例："必须在3天内完成" → windowEnd包含容差信息

5. **默认值填充**：如果方案中没有明确时间窗，根据访视类型推断
   - 筛选期：windowStart: "Day -28", windowEnd: "Day -1"
   - 基线：windowStart: "Day 0", windowEnd: "Day 0"
   - 治疗周期：使用周期标识（如"C1D1"）
   - 随访：windowStart和windowEnd都填"随访期"
